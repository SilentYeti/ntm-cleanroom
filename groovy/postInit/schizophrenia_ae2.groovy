...
// ============================================================================
// NTM machine recipes for the AE2 progression, transferred out of raw JSON
// config into here so they're live-editable via /gs reload instead of
// requiring a restart.
//
// These 4 files each had their AE2 (and, for Assembly Machine, ntmcleanroom-
// prefixed custom-mod) recipes stripped out of the on-disk JSON, leaving
// only hbm's own stock catalog there. The stock catalog stays wherever it
// was (config/hbmRecipes/<file>.json); the recipes that used to live at the
// tail of those files are now hard-coded below and re-inserted via
// mods.hbm.recipeOverrides.override(target, json) every script run - same
// "read the live file fresh, splice in text after the recipes array opens"
// approach as the Assembly Machine placeholder above, since groovy-json
// isn't bundled in this GroovyScript build (see that section's note).
//
// TARGET NAMES NOT VERIFIED against a live session for hbmArcWelder/
// hbmSoldering/hbmChemicalPlant, same caveat as hbmAssemblyMachine above -
// run `log.info(mods.hbm.recipeOverrides.listTargets())` once if any of
// these error, and check logs/groovy.log for the accepted name.
// ============================================================================
...
// --- Arc Welder: combine each AE2 Part + an Empty Storage Cell (material:39)
//     into the finished storage/spatial cell ---
...
// --- Soldering: the Press+Soldering chain that replaces the Inscriber's
//     Logic/Calculation/Engineering Processor step (Silicon Print + a
//     Processor Print + redstone -> the finished Processor) ---
